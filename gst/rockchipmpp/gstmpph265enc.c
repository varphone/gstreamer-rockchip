...
typedef enum
{
  GST_MPP_H265_PROFILE_BASELINE = 66,
  GST_MPP_H265_PROFILE_MAIN = 77,
  GST_MPP_H265_PROFILE_HIGH = 100,
} GstMppH265Profile;
...
#define DEFAULT_PROP_PROFILE GST_MPP_H265_PROFILE_HIGH
...
//       {GST_MPP_H265_PROFILE_BASELINE, "Baseline", "baseline"},
//       {GST_MPP_H265_PROFILE_MAIN, "Main", "main"},
//       {GST_MPP_H265_PROFILE_HIGH, "High", "high"},
//       {0, NULL, NULL},
//     };
    ...
    guint prop_id, const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    case PROP_PROFILE:{
      GstMppH265Profile profile = g_value_get_enum (value);
...
gst_mpp_h265_enc_set_src_caps (GstVideoEncoder * encoder)
{
  ...
  // string = g_enum_to_string (GST_TYPE_MPP_H265_ENC_PROFILE, self->profile);
  // gst_structure_set (structure, "profile", G_TYPE_STRING, string, NULL);
  // g_free (string);

  // string = g_enum_to_string (GST_TYPE_MPP_H265_ENC_LEVEL, self->level);
...
gst_mpp_h265_enc_apply_properties (GstVideoEncoder * encoder)
{
  ...
  mpp_enc_cfg_set_s32 (mppenc->mpp_cfg, "codec:type", MPP_VIDEO_CodingHEVC);

  // mpp_enc_cfg_set_s32 (mppenc->mpp_cfg, "h265:profile", self->profile);
  // mpp_enc_cfg_set_s32 (mppenc->mpp_cfg, "h265:level", self->level);

  // mpp_enc_cfg_set_s32 (mppenc->mpp_cfg, "h265:trans8x8",
  //     self->profile == GST_MPP_H265_PROFILE_HIGH);
  // mpp_enc_cfg_set_s32 (mppenc->mpp_cfg, "h265:cabac_en",
  //     self->profile != GST_MPP_H265_PROFILE_BASELINE);
  // mpp_enc_cfg_set_s32 (mppenc->mpp_cfg, "h265:cabac_idc", 0);
...
gst_mpp_h265_enc_class_init (GstMppH265EncClass * klass)
{
  ...
  //         "H265 level (40~41 = 1080p@30fps, 42 = 1080p60fps, 50~52 = 4K@30fps)",
